...
export class TimelyApp {
    ...
    constructor(_config: TimelyAppConfig) {
        if (!_config.accountId || !_config.token)
    ...
    async getAccountById(accountId: string): Promise<TimelyAccount> {
        const { data: response }: { data: TimelyAccount } = await this._request(
            `/accounts/${accountId}`,
        )
        return response
    }
    ...
    async getEventsByProjectId(
        ...
        const { data: response }: { data: TimelyEvent[] } = await this._request(
            `/${this._config.accountId}/projects/${projectId}/events${
                start ? `?since=${start}` : ''
            }${end ? `&upto=${end}` : ''}`,
        )
        return response
  ...
  async updateEventById(eventId: number, event: TimelyEvent): Promise<TimelyEvent> {
    ...
    async getProjects(): Promise<TimelyProject[]> {
        const { data: response }: { data: TimelyProject[] } = await this._request(
            `/${this._config.accountId}/projects`,
        )
        return response
    }
    ...
    async getRoles(): Promise<TimelyRole[]> {
        const { data: response }: { data: TimelyRole[] } = await this._request(
            `/${this._config.accountId}/roles`,
        )
        return response
    }
